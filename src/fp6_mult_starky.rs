...
use crate::native::{
    ...
    multiply_by_slice, sub_u32_slices, Fp, Fp2, calc_qs, calc_precomp_stuff_loop0, sub_u32_slices_12, mul_u32_slice_u32, mod_inverse, get_bls_12_381_parameter, calc_precomp_stuff_loop1, Fp6,
};
...
pub const FP6_MUL_X_INPUT_OFFSET: usize = 0;
...
pub const FP6_TOTAL_COLUMNS: usize = FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_TOTAL + (FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL)*2;


pub const TOTAL_COLUMNS: usize = FP6_TOTAL_COLUMNS;
pub const COLUMNS: usize = FP6_TOTAL_COLUMNS;

pub const PUBLIC_INPUT_X_OFFSET: usize = 0;
pub const PUBLIC_INPUT_Y_OFFSET: usize = PUBLIC_INPUT_X_OFFSET + 24*3;
pub const PUBLIC_INPUT_SUM_OFFSET: usize = PUBLIC_INPUT_Y_OFFSET + 24*3;
pub const PUBLIC_INPUTS: usize = PUBLIC_INPUT_SUM_OFFSET + 24*3;
...
// A (Fp) * B (Fp) => C (Fp)
#[derive(Clone)]
pub struct Fp6MulStark<F: RichField + Extendable<D>, const D: usize> {
    pub trace: Vec<[F; TOTAL_COLUMNS]>,
    num_rows: usize,
}

// Implement trace generator
impl<F: RichField + Extendable<D>, const D: usize> Fp6MulStark<F, D> {
    ...
    pub fn generate_trace(&mut self, x: Fp6, y: Fp6) {
        for i in 0..6 {
            for row in 0..self.num_rows {
                self.assign_u32_in_series(row, FP6_MUL_X_INPUT_OFFSET + 12*i, &x.0[i].0);
                self.assign_u32_in_series(row, FP6_MUL_Y_INPUT_OFFSET + 12*i, &y.0[i].0);
            }
        }
        ...
        let t0 = c0*r0;
        self.generate_trace_fp2_mul(c0.get_u32_slice(), r0.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T0_CALC_OFFSET);
        let t1 = c1*r1;
        self.generate_trace_fp2_mul(c1.get_u32_slice(), r1.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T1_CALC_OFFSET);
        let t2 = c2*r2;
        self.generate_trace_fp2_mul(c2.get_u32_slice(), r2.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T2_CALC_OFFSET);

        let t3 = c1+c2;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&c1.get_u32_slice(), &c2.get_u32_slice(), row, FP6_MUL_T3_CALC_OFFSET);
        }
        let t4 = r1+r2;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&r1.get_u32_slice(), &r2.get_u32_slice(), row, FP6_MUL_T4_CALC_OFFSET);
        }
        let t5 = t3*t4;
        self.generate_trace_fp2_mul(t3.get_u32_slice(), t4.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T5_CALC_OFFSET);
        let t6 = t5-t1;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t5.get_u32_slice(), &t1.get_u32_slice(), row, FP6_MUL_T6_CALC_OFFSET);
        }
        let t7 = t6-t2;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t6.get_u32_slice(), &t2.get_u32_slice(), row, FP6_MUL_T7_CALC_OFFSET);
        }
        let t8 = t7.mul_by_nonresidue();
        for row in 0..self.num_rows {
            self.fill_trace_non_residue_multiplication(&t7.get_u32_slice(), row, FP6_MUL_T8_CALC_OFFSET);
        }
        let _x = t8+t0;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&t8.get_u32_slice(), &t0.get_u32_slice(), row, FP6_MUL_X_CALC_OFFSET);
        }

        let t9 = c0+c1;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&c0.get_u32_slice(), &c1.get_u32_slice(), row, FP6_MUL_T9_CALC_OFFSET);
        }
        let t10 = r0+r1;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&r0.get_u32_slice(), &r1.get_u32_slice(), row, FP6_MUL_T10_CALC_OFFSET);
        }
        let t11 = t9*t10;
        self.generate_trace_fp2_mul(t9.get_u32_slice(), t10.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T11_CALC_OFFSET);
        let t12 = t11-t0;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t11.get_u32_slice(), &t0.get_u32_slice(), row, FP6_MUL_T12_CALC_OFFSET);
        }
        let t13 = t12-t1;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t12.get_u32_slice(), &t1.get_u32_slice(), row, FP6_MUL_T13_CALC_OFFSET);
        }
        let t14 = t2.mul_by_nonresidue();
        for row in 0..self.num_rows {
            self.fill_trace_non_residue_multiplication(&t2.get_u32_slice(), row, FP6_MUL_T14_CALC_OFFSET);
        }
        let _y = t13+t14;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&t13.get_u32_slice(), &t14.get_u32_slice(), row, FP6_MUL_Y_CALC_OFFSET);
        }

        let t15 = c0+c2;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&c0.get_u32_slice(), &c2.get_u32_slice(), row, FP6_MUL_T15_CALC_OFFSET);
        }
        let t16 = r0+r2;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&r0.get_u32_slice(), &r2.get_u32_slice(), row, FP6_MUL_T16_CALC_OFFSET);
        }
        let t17 = t15*t16;
        self.generate_trace_fp2_mul(t15.get_u32_slice(), t16.get_u32_slice(), 0, self.num_rows-1, FP6_MUL_T17_CALC_OFFSET);
        let t18 = t17-t0;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t17.get_u32_slice(), &t0.get_u32_slice(), row, FP6_MUL_T18_CALC_OFFSET);
        }
        let t19 = t18-t2;
        for row in 0..self.num_rows {
            self.fill_trace_subtraction_with_reduction(&t18.get_u32_slice(), &t2.get_u32_slice(), row, FP6_MUL_T19_CALC_OFFSET);
        }
        let _z = t19+t1;
        for row in 0..self.num_rows {
            self.fill_trace_addition_with_reduction(&t19.get_u32_slice(), &t1.get_u32_slice(), row, FP6_MUL_Z_CALC_OFFSET);
        }
    }
...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for Fp6MulStark<F, D> {
        ...
        P: PackedField<Scalar = FE>,
    {
        ...
        // ----
        for i in 0..24*3 {
            yield_constr.constraint_first_row(local_values[FP6_MUL_X_INPUT_OFFSET + i] - public_inputs[PUBLIC_INPUT_X_OFFSET + i]);
            yield_constr.constraint_first_row(local_values[FP6_MUL_Y_INPUT_OFFSET + i] - public_inputs[PUBLIC_INPUT_Y_OFFSET + i]);
        }
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i + 12])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i + 24])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i + 36])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i + 48])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] *
                (local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - public_inputs[PUBLIC_INPUT_SUM_OFFSET + i + 60])
            );
        }

        for i in 0..24*3 {
            yield_constr.constraint_transition(local_values[FP6_MUL_X_INPUT_OFFSET + i] - next_values[FP6_MUL_X_INPUT_OFFSET + i]);
            yield_constr.constraint_transition(local_values[FP6_MUL_Y_INPUT_OFFSET + i] - next_values[FP6_MUL_Y_INPUT_OFFSET + i]);
        }

        // T0
        for i in 0..24 {
            yield_constr.constraint(
                local_values[FP6_MUL_T0_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_X_INPUT_OFFSET + i] - local_values[FP6_MUL_T0_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T0_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_Y_INPUT_OFFSET + i] - local_values[FP6_MUL_T0_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T0_CALC_OFFSET);

        // T1
        for i in 0..24 {
            yield_constr.constraint(
                local_values[FP6_MUL_T1_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_X_INPUT_OFFSET + i + 24] - local_values[FP6_MUL_T1_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T1_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_Y_INPUT_OFFSET + i + 24] - local_values[FP6_MUL_T1_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T1_CALC_OFFSET);

        // T2
        for i in 0..24 {
            yield_constr.constraint(
                local_values[FP6_MUL_T2_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_X_INPUT_OFFSET + i + 48] - local_values[FP6_MUL_T2_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T2_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_Y_INPUT_OFFSET + i + 48] - local_values[FP6_MUL_T2_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T2_CALC_OFFSET);

        // T3
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 24]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 24 + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 48]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 48 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T3_CALC_OFFSET);

        // T4
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 24]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 24 + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 48]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 48 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T4_CALC_OFFSET);

        // T5
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T3_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i + 12])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T4_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T5_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i + 12])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T5_CALC_OFFSET);

        // T6
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T5_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T5_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T6_CALC_OFFSET);

        // T7
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T6_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T2_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T2_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T7_CALC_OFFSET);

        // T8
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_CHECK_OFFSET] *
                (
                    local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_INPUT_OFFSET + i] - local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_CHECK_OFFSET] *
                (
                    local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_INPUT_OFFSET + i + 12] - local_values[FP6_MUL_T7_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
        }
        add_non_residue_multiplication_constraints(local_values, yield_constr, FP6_MUL_T8_CALC_OFFSET);

        // X calc offset
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_Z0_REDUCE_OFFSET + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T8_CALC_OFFSET + FP2_NON_RESIDUE_MUL_Z1_REDUCE_OFFSET + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_X_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_X_CALC_OFFSET);

        // T9
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 24]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 24 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T9_CALC_OFFSET);

        // T10
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 24]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 24 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T10_CALC_OFFSET);

        // T11
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T9_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i + 12])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T10_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T11_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i + 12])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T11_CALC_OFFSET);

        // T12
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T11_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T11_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T12_CALC_OFFSET);

        // T13
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T12_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T13_CALC_OFFSET);

        // T14
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_CHECK_OFFSET] *
                (
                    local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_INPUT_OFFSET + i] - local_values[FP6_MUL_T2_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_CHECK_OFFSET] *
                (
                    local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_INPUT_OFFSET + i + 12] - local_values[FP6_MUL_T2_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }
        add_non_residue_multiplication_constraints(local_values, yield_constr, FP6_MUL_T14_CALC_OFFSET);

        // Y calc offset
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T13_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_Z0_REDUCE_OFFSET + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Y_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T14_CALC_OFFSET + FP2_NON_RESIDUE_MUL_Z1_REDUCE_OFFSET + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_Y_CALC_OFFSET);

        // T15
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 48]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_X_INPUT_OFFSET + i + 48 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T15_CALC_OFFSET);

        // T16
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 12]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 48]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_Y_INPUT_OFFSET + i + 48 + 12]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T16_CALC_OFFSET);

        // T17
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T15_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_X_INPUT_OFFSET + i + 12])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i])
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_SELECTOR_OFFSET] *
                (local_values[FP6_MUL_T16_CALC_OFFSET + FP2_ADDITION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i] - local_values[FP6_MUL_T17_CALC_OFFSET + FP2_FP2_Y_INPUT_OFFSET + i + 12])
            );
        }
        add_fp2_mul_constraints(local_values, next_values, yield_constr, FP6_MUL_T17_CALC_OFFSET);

        // T18
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T17_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T17_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T0_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T18_CALC_OFFSET);

        // T19
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T18_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_0_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T2_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_1_OFFSET + FP_SUBTRACTION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T2_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_subtraction_with_reduction_constranints(local_values, yield_constr, FP6_MUL_T19_CALC_OFFSET);

        // Z calc offset
        for i in 0..12 {
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_X_OFFSET + i] -
                    local_values[FP6_MUL_T19_CALC_OFFSET + FP2_ADDITION_TOTAL + FP2_SUBTRACTION_TOTAL + FP_SINGLE_REDUCE_TOTAL + RANGE_CHECK_TOTAL + FP_SINGLE_REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_0_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z1_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
            yield_constr.constraint(
                local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_CHECK_OFFSET] * (
                    local_values[FP6_MUL_Z_CALC_OFFSET + FP2_ADDITION_1_OFFSET + FP_ADDITION_Y_OFFSET + i] -
                    local_values[FP6_MUL_T1_CALC_OFFSET + Z2_REDUCE_OFFSET + REDUCED_OFFSET + i]
                )
            );
        }

        add_addition_with_reduction_constranints(local_values, yield_constr, FP6_MUL_Z_CALC_OFFSET);
    }
