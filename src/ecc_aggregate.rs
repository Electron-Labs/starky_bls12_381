...
use plonky2::{field::{extension::{Extendable, FieldExtension}, packed::PackedField}, hash::hash_types::RichField, iop::ext_target::ExtensionTarget};
use starky::{constraint_consumer::ConstraintConsumer, evaluation_frame::{StarkEvaluationFrame, StarkFrame}, stark::Stark};
...
impl<F: RichField + Extendable<D>, const D: usize> ECCAggStark<F, D> {
    ...
    pub fn generate_trace(&self, points: &[[Fp; 2]], bits: &[bool]) -> Vec<[F; TOTAL_COLUMNS]> {
        ...
        assert!(num_rows_req < self.num_rows, "stark doesn't have enough rows");
        ...
        for i in 0..NUM_POINTS {
            if i < 2 {
                (row..row+12).into_iter().for_each(|rw| trace[rw][PIS_IDX + i] = F::ONE);
            } else {
                row += 12;
                (row..row+12).into_iter().for_each(|rw| trace[rw][PIS_IDX + i] = F::ONE);
            }
...
impl<F: RichField + Extendable<D>, const D: usize> Stark<F, D> for ECCAggStark<F, D> {
        ...
        P: PackedField<Scalar = FE>,
    {
        ...
        for i in 0..12 {
            if i < 11 {
                yield_constr.constraint_transition(local_values[ROW_NUM + i] - next_values[ROW_NUM + i + 1]);
            } else {
                yield_constr.constraint_transition(local_values[ROW_NUM + i] - next_values[ROW_NUM]);
            }
        }
        ...
        for idx in 2..NUM_POINTS {
            for i in 0..12 {
                yield_constr.constraint_transition(
                    ...
                    (next_values[OP + G1_POINT_ADDITION_X2 + i] - public_inputs[POINTS + 24*idx + i])
                );
                yield_constr.constraint_transition(
                    next_values[ROW_NUM] *
                    next_values[PIS_IDX + idx] *
        ...
        for i in 0..12 {
            yield_constr.constraint_transition(
                ...
                (local_values[OP + G1_POINT_ADDITION_X1 + i] - next_values[OP + G1_POINT_ADDITION_X1 + i])
            );
            yield_constr.constraint_transition(
                (P::ONES - next_values[ROW_NUM]) *
                (local_values[OP + G1_POINT_ADDITION_Y1 + i] - next_values[OP + G1_POINT_ADDITION_Y1 + i])
            );
            yield_constr.constraint_transition(
                (P::ONES - next_values[ROW_NUM]) *
                (local_values[OP + G1_POINT_ADDITION_X2 + i] - next_values[OP + G1_POINT_ADDITION_X2 + i])
            );
            yield_constr.constraint_transition(
                (P::ONES - next_values[ROW_NUM]) *
                (local_values[OP + G1_POINT_ADDITION_Y2 + i] - next_values[OP + G1_POINT_ADDITION_Y2 + i])
            );
            yield_constr.constraint_transition(
                (P::ONES - next_values[ROW_NUM]) *
                (local_values[OP + G1_POINT_ADDITION_X3 + i] - next_values[OP + G1_POINT_ADDITION_X3 + i])
            );
            yield_constr.constraint_transition(
                (P::ONES - next_values[ROW_NUM]) *
                (local_values[OP + G1_POINT_ADDITION_Y3 + i] - next_values[OP + G1_POINT_ADDITION_Y3 + i])
            );
        }
        ...
        yield_constr.constraint_transition(
            (P::ONES - next_values[ROW_NUM]) *
            (local_values[A_IS_INF] - next_values[A_IS_INF])
        );
        yield_constr.constraint_transition(
            (P::ONES - next_values[ROW_NUM]) *
            (local_values[B_IS_INF] - next_values[B_IS_INF])
        );
        ...
        for i in 0..12 {
            yield_constr.constraint_transition(
                ...
                (
                    ...
                    (P::ONES - local_values[A_IS_INF] - local_values[B_IS_INF])*local_values[OP + G1_POINT_ADDITION_X3 + i] -
            ...
            yield_constr.constraint_transition(
                ...
                (
                    ...
                    (P::ONES - local_values[A_IS_INF] - local_values[B_IS_INF])*local_values[OP + G1_POINT_ADDITION_Y3 + i] -
    ...
    fn eval_ext_circuit(
        ...
        for i in 0..12 {
            if i < 11 {
                let c = builder.sub_extension(local_values[ROW_NUM + i], next_values[ROW_NUM + i + 1]);
        ...
        for i in 1..NUM_POINTS - 1 {
            ...
            let sub2 = builder.sub_extension(local_values[PIS_IDX + i], next_values[PIS_IDX + i + 1]);
            let c = builder.mul_extension(mul, sub2);
            yield_constr.constraint_transition(builder, c);
        }

        for i in 0..NUM_POINTS {
            let mul = builder.mul_extension(local_values[PIS_IDX + NUM_POINTS - 1], next_values[ROW_NUM]);
            let c = builder.mul_extension(mul, next_values[PIS_IDX + i]);
            yield_constr.constraint_transition(builder, c);
        }

        for i in 0..12 {
            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_X1 + i], public_inputs[POINTS + i]);
            yield_constr.constraint_first_row(builder, c);
            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_Y1 + i], public_inputs[POINTS + i + 12]);
            yield_constr.constraint_first_row(builder, c);
            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_X2 + i], public_inputs[POINTS + 24 + i]);
            yield_constr.constraint_first_row(builder, c);
            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_Y2 + i], public_inputs[POINTS + 24 + i + 12]);
            yield_constr.constraint_first_row(builder, c);
        }
        ...
        for idx in 2..NUM_POINTS {
            for i in 0..12 {
                ...
                let c = builder.sub_extension(next_values[OP + G1_POINT_ADDITION_X2 + i], public_inputs[POINTS + 24*idx + i]);
                let c = builder.mul_extension(mul, c);
                yield_constr.constraint_transition(builder, c);

                let c = builder.sub_extension(next_values[OP + G1_POINT_ADDITION_Y2 + i], public_inputs[POINTS + 24*idx + i + 12]);
                let c = builder.mul_extension(mul, c);
                yield_constr.constraint_transition(builder, c);
            }
            let one = builder.one_extension();
        ...
        for i in 0..12 {
            ...
            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_X1 + i], next_values[OP + G1_POINT_ADDITION_X1 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);

            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_Y1 + i], next_values[OP + G1_POINT_ADDITION_Y1 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);

            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_X2 + i], next_values[OP + G1_POINT_ADDITION_X2 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);

            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_Y2 + i], next_values[OP + G1_POINT_ADDITION_Y2 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);

            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_X3 + i], next_values[OP + G1_POINT_ADDITION_X3 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);

            let c = builder.sub_extension(local_values[OP + G1_POINT_ADDITION_Y3 + i], next_values[OP + G1_POINT_ADDITION_Y3 + i]);
            let c = builder.mul_extension(sub1, c);
            yield_constr.constraint_transition(builder, c);
        }
        ...
        for i in 0..12 {
            ...
            let mul1 = builder.mul_extension(local_values[A_IS_INF], local_values[OP + G1_POINT_ADDITION_X2 + i]);
            let mul2 = builder.mul_extension(local_values[B_IS_INF], local_values[OP + G1_POINT_ADDITION_X1 + i]);
            ...
            let mul3 = builder.mul_extension(res_selector, local_values[OP + G1_POINT_ADDITION_X3 + i]);
            ...
            let mul1 = builder.mul_extension(local_values[A_IS_INF], local_values[OP + G1_POINT_ADDITION_Y2 + i]);
            let mul2 = builder.mul_extension(local_values[B_IS_INF], local_values[OP + G1_POINT_ADDITION_Y1 + i]);
            let a_b_inf_or = builder.add_extension(local_values[A_IS_INF], local_values[B_IS_INF]);
            let res_selector = builder.sub_extension(one, a_b_inf_or);
            let mul3 = builder.mul_extension(res_selector, local_values[OP + G1_POINT_ADDITION_Y3 + i]);
            let sel = builder.add_many_extension([mul1, mul2, mul3].iter());
        ...
        add_g1_addition_constraints_ext_circuit(builder, yield_constr, local_values, next_values, OP, None);
...
mod tests {
    ...
    use plonky2::{field::types::Field, plonk::config::{GenericConfig, PoseidonGoldilocksConfig}, util::timing::TimingTree};
    use starky::{config::StarkConfig, prover::prove, stark_testing::test_stark_circuit_constraints, util::trace_rows_to_poly_values, verifier::verify_stark_proof};

    use crate::{ecc_aggregate::{ECCAggStark, PUBLIC_INPUTS}, native::Fp};
