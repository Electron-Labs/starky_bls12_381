...
>(
    ...
    a.limbs.iter().zip(b.limbs.iter()).fold(builder.constant_bool(true), |acc, (a_l, b_l)| {
        let is_equal = builder.is_equal(a_l.0, b_l.0);
        builder.and(acc, is_equal)
    })
}

pub fn range_check_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    input: &FpTarget,
...
pub fn add_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    a: &FpTarget,
    ...
    let sub_limbs = (0..12).into_iter().map(|i| U32Target(builder.select(cmp, p.limbs[i].0, zero))).collect::<Vec<U32Target>>();
    let sub = BigUintTarget { limbs: sub_limbs };
    builder.sub_biguint(&res, &sub)
}

pub fn negate_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    input: &FpTarget,
...
pub fn sub_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    a: &FpTarget,
...
pub fn mul_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    a: &FpTarget,
...
pub fn inv_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    input: &FpTarget,
...
pub fn div_fp<F: RichField + Extendable<D>,
    const D: usize
>(
    builder: &mut CircuitBuilder<F, D>,
    a: &FpTarget,
...
impl<F: RichField + Extendable<D>, const D: usize> SimpleGenerator<F, D> for FpInverseGenerator {
    ...
    fn dependencies(&self) -> Vec<Target> {
        self.input.limbs.iter().map(|l| l.0).collect::<Vec<Target>>()
    }
